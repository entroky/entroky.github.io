...
const broadcastFileChange = async (fileName, token) => {
    if (fileName === "combined_style")
        return;
    try {
        const contents = await fs.readFile(fileName, "utf8");
        if (wss) {
            for (const ws of wss.clients) {
                ws.send(JSON.stringify({ kind: "file_change", fileName, contents, token }));
            }
        }
        else {
            console.warn("Websocket server not defined");
...
export default async function (port = 9160) {
    ...
    watcher.on("add", (p) => {
        switch (p.split(".").pop()) {
            ...
            case "json":
                ...
                if (p.split(".").slice(1).join(".") === "trio.json")
                    files.trio.push(p);
        }
        broadcastFiles();
    });
    ...
    watcher.on("change", async (p) => {
        if (["json", "substance", "style", "domain"].includes(p.split(".").pop() ?? "")) {

...
const render = async (variation, substance, style, domain, resolvePath, texLabels, verbose, meta, excludeWarnings) => {
    // create id for reporting
    // Compilation
    if (verbose)
        console.debug(`Compiling ${meta.id} ...`);
    ...
    if (verbose)
        console.debug(`Stepping for ${meta.id} ...`);
...
const resolvePath = (prefix, stylePaths) => {
    ...
    if (new Set(stylePrefixes).size > 1) {
        console.warn(chalk.yellow("Warning: the styles in this trio are not co-located. The first style will be used for image resolution."));
    }
...
const readTrio = (sub, sty, dsl, prefix) => {
    // Fetch Substance, Style, and Domain files
    const [substance, domain] = [sub, dsl].map((arg) => fs.readFileSync(join(prefix, arg), "utf8"));
...
const orderTrio = (unordered) => {
    ...
    for (const fakeType in unordered) {
        ...
        if (type in ordered) {
            console.error(`Duplicate ${type} files: ${ordered[type]} and ${filename}`);
            process.exit(1);
        }
...
yargs(hideBin(process.argv))
    ...
    .command("trio [trio..]", "Generate a diagram from a Penrose trio.", (yargs) => yargs
    .options("trio", {
    ...
    let excludeWarnings = undefined;
    if (options.trio.length === 1) {
        const trioPath = options.trio[0];
        prefix = join(trioPath, "..");
        // read trio from a JSON file
        const paths = JSON.parse(fs.readFileSync(resolve(trioPath), "utf8"));
        dom = paths.domain;
        sub = paths.substance;
        sty = paths.style;
        ...
        const trio = orderTrio(options.trio);
    ...
    const { diagram, state } = await render(variation ?? "", substance, style, domain, resolvePath(prefix, sty), texLabels, false, {
        substanceName: sub,
        styleNames: sty,
        domainName: dom,
    ...
    if (options.out) {
        ...
        console.log(chalk.green(`The diagram has been saved as ${resolve(options.out)}`));
    }
    else {
        console.log(diagram);
    ...
    .command("trios [trios..]", "Create diagrams from multiple .trio.json files.", (yargs) => yargs
    .options("trios", {
    ...
    for (const trio of options.trios) {
        ...
        const excludeWarnings = paths.excludeWarnings === undefined ? [] : paths.excludeWarnings;
        const { substance, style, domain } = readTrio(sub, sty, dom, prefix);
        const { diagram, state } = await render(variation, substance, style, domain, resolvePath(prefix, sty), texLabels, false, {
            substanceName: sub,
            styleNames: sty,
        ...
        console.log(chalk.green(`The diagram has been saved as ${resolve(outputPath)}`));
    ...
    .command("watch", "Watch the current folder for files & changes (must end in .substance, .style, .domain)", (yargs) => yargs.option("port", {
